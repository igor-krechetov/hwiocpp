...
void DeviceGPIO::closeDevice()
{
    ...
    if (true == isDeviceOpen())
    {
        ...
        if (sOpenChips.end() != itChip)
        {
            ...
            {
                __TRACE_DEBUG__("wait for monitoring thread to finish...");
        ...
        {
            __TRACE_ERROR__("current chip is not found. are you using DeviceGPIO from multiple threads?");
        }
    }
}
...
{
    __TRACE_CALL_DEBUG_ARGS__("pin=%d, pullMode=%d", SC2INT(pin), SC2INT(pullMode));
    ...
    if ((mGroups.end() != itGroup) && (itPins != mGroupPins.end()))
    {
        if (true == changeGroupDirection(itGroup->first, GPIO_PIN_MODE::INPUT))
        {
            ...
            if (0 == gpiod_line_get_value_bulk(&(itGroup->second), outValues.data()))
            {
                ...
                {
                    __TRACE_DEBUG__("[%d] = %d", i, outValues[i]);
                }
...
{
    __TRACE_CALL_DEBUG_ARGS__("pin=%d, mode=%d, pullMode=%d", SC2INT(pin), SC2INT(mode), SC2INT(pullMode));
    bool result = false;
    auto itPin = mActiveLines.find(pin);
            ...
            if (true == setPinPullMode(pin, pullMode))
            {
                ...
                if (mode != GPIO_PIN_MODE::EDGE_DETECTION)
                {
                    ...
                    else
                    {
                        ...
                        result = false;
                        __TRACE_ERROR__("Request line failed");
                    }
                }
                else
                {
...
bool DeviceGPIO::startEdgeEventsMonitorining(const RP_GPIO pin)
{
    ...
    if (mEdgeCallback)
    {
        __TRACE_CALL_ARGS__("pin=%d (v2)", SC2INT(pin));
        auto itPin = mActiveLines.find(pin);
        ...
        if (itPin != mActiveLines.end())
        {
            ...
            {
                __TRACE_ERROR__("gpiod_line_request_bulk_both_edges_events failed");
            }
        }
    }
...
{
    __TRACE_CALL_ARGS__("groupID=%d", SC2INT(groupID));
...
bool DeviceGPIO::changePinDirection(const RP_GPIO pin, const GPIO_PIN_MODE direction)
{
    ...
    if (itPin != mActiveLines.end())
    {
        if (direction != itPin->second.mode)
        {
            ...
            {
                __TRACE_ERROR__("changing pin direction failed. closingPin");
                closePin(pin);
            }
        }
        else
...
bool DeviceGPIO::changePinPullMode(const RP_GPIO pin, const GPIO_PIN_PULL pullMode)
{
    ...
    if (itPin != mActiveLines.end())
    {
        if ((GPIO_PIN_PULL::AS_IS != pullMode) && (pullMode != itPin->second.pull))
        {
            ...
            {
                __TRACE_ERROR__("changing pin pull mode failed");
            }
        }
        else
        {
        ...
        if ((gpiod_line_bulk_num_lines(&monitoringBulk) > 0) && (nullptr != mEdgeCallback))
        {
            ...
            if (rc > 0)
            {
                ...
                for (unsigned int i = 0 ; i < linesCount; ++i)
                {
                    ...
                    {
                        __TRACE_DEBUG__("pin=%d, event=%d", SC2INT(pin), eventInfo.event_type);
            ...
            {
                __TRACE_ERROR__("gpiod_line_event_wait_bulk -> error");
            }
        }
        else
        {
            __TRACE_DEBUG__("no pins to monitor - exit thread");
            break;
        }
    }
